...
pub async fn update_visa_details(
    ...
    let mut tx = pool.begin().await.map_err(|e| {
        eprintln!("❌ Failed to begin transaction in update_visa_details: {}", e);
        eprintln!("❌ Transaction error details: {:?}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
    
    let select_sql = "SELECT * FROM visa_db.h1bcustomer WHERE email = $1";
    ...
    tx.commit().await.map_err(|e| {
        eprintln!("❌ Failed to commit transaction in update_visa_details: {}", e);
...
pub async fn delete_visa_details(
    ...
    let pool = get_db_pool();
    let mut tx = pool.begin().await.map_err(|e| {
        eprintln!("❌ Failed to begin transaction in delete_visa_details: {}", e);
        eprintln!("❌ Transaction error details: {:?}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
    
    let query_sql = "DELETE FROM visa_db.h1bcustomer WHERE email = $1";
    ...
    let result = sqlx::query(&query_sql)
        ...
        .map_err(|e| {
            ...
            eprintln!("❌ Delete error details: {:?}", e);
            eprintln!("❌ SQL Query: {}", query_sql);
            eprintln!("❌ Email: {}", email);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    ...
    tx.commit().await.map_err(|e| {
        eprintln!("❌ Failed to commit transaction in delete_visa_details: {}", e);
        eprintln!("❌ Commit error details: {:?}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
...
pub async fn get_all_h1b_customers() -> Result<Json<Vec<CreateCustomer>>, StatusCode> {
    ...
    let pool = get_db_pool();
    let mut tx = pool.begin().await.map_err(|e| {
        eprintln!("❌ Failed to begin transaction in get_all_h1b_customers: {}", e);
        eprintln!("❌ Transaction error details: {:?}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
    ...
    let query_sql = "SELECT customer_id, email, first_name, last_name, dob, sex::text, marital_status::text, phone, 
        ...
        client_name, client_street_name, client_city, client_state, client_zip,
        lca_title, lca_salary, lca_code, receipt_number, h1b_start_date, h1b_end_date, h1b_status::text
        FROM visa_db.h1bcustomer WHERE h1b_status = 'Active'";
    ...
    tx.commit().await.map_err(|e| {
        eprintln!("❌ Failed to commit transaction in get_all_h1b_customers: {}", e);
        eprintln!("❌ Commit error details: {:?}", e);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
...
pub async fn soft_delete_customer_by_id(
    ...
    match sqlx::query("UPDATE visa_db.h1bcustomer SET h1b_status = 'Inactive' WHERE customer_id = $1::uuid")
        .bind(&customer_id)
        .execute(&pool)
        .await 
    {
